...
def page_summary():
    ...
    st.header("1. 전자기 유도와 유도 기전력")
    st.markdown("""
    * **전자기 유도:** 코일과 자석의 상대적인 운동(운동 에너지)으로 도체 주변에 자기장이 변하여 전원 공급 없이 전류(전기 에너지)가 흐르게 되는 현상입니다[cite: 1].
    * **유도 기전력:** 전자기 유도 현상에 의해 전류를 흐르게 하는 능력으로, 회로에서 전압의 역할을 합니다[cite: 1].
    """)
    
    st.header("2. 유도 전류의 세기가 커지는 조건")
    st.markdown("""
    * **유도 기전력이 커지는 경우:** 더 강한 자석을 사용하거나, 코일을 더 많이 감거나, 코일 또는 자석을 더 빠르게 움직일 때 커집니다[cite: 1]. 단, 코일과 자석의 상대적 위치가 변해야 합니다[cite: 1].
    * **저항이 작아지는 경우:** 코일 외부에 저항값이 작은 저항이나 전구 등을 사용할 때 유도 전류의 세기가 커집니다[cite: 1].
    """)
    
    st.header("3. 유도 전류의 방향 찾기 원리")
    st.markdown("""
    * 전자기 유도는 자기장 변화를 방해하기 위한 방향으로 발생합니다[cite: 1].
    * 전류가 흐르는 코일은 자석과 같은 역할을 합니다[cite: 1].
    * 유도 전류의 방향에 영향을 주는 경우의 수로는 자석의 N극/S극 여부, 가까워짐/멀어짐 여부, 그리고 코일을 감은 방향이 있습니다[cite: 1].
    * 이 조건들 중 홀수개의 조건이 반대가 되면 유도 전류의 방향은 반대가 됩니다[cite: 1].
    """)
    
    st.header("4. 코일과 자석의 상호작용 (4가지 Case)")
    st.markdown("""
    | 자석의 움직임 | 작용하는 힘(자기력) | 변화 방해 원리 | 유도된 코일의 극(위쪽) |
    | :--- | :--- | :--- | :--- |
    | **N극이 가까워짐 (Case 1)** | 척력 (밀어냄)[cite: 2] | 같은 극이 자석을 밀어냄[cite: 2] | N극[cite: 2] |
    | **S극이 가까워짐 (Case 2)** | 척력 (밀어냄)[cite: 2] | 같은 극이 자석을 밀어냄[cite: 2] | S극[cite: 2] |
    | **N극이 멀어짐 (Case 3)** | 인력 (끌어당김)[cite: 2] | 반대 극이 자석을 끌어당김[cite: 2] | S극[cite: 2] |
    | **S극이 멀어짐 (Case 4)** | 인력 (끌어당김)[cite: 2] | 반대 극이 자석을 끌어당김[cite: 2] | N극[cite: 2] |
    """)
